...
# Загружаем намерения из intents.json
...
class UserState:
    ...
    def __init__(self):
        ...
        self.grades = {}  # Словарь оценок {предмет: оценка} (None если нет оценки)
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # ========== ЭТАП 2: ОЦЕНКИ ==========
        ...
        if text.lower() in ["нет", "не было"]:
            prep_subject = get_subject_in_prepositional(subject)
            await update.message.reply_text(f"Принято. Оценки по {prep_subject} не было.")
            state.grades[subject] = None  # Сохраняем None как признак отсутствия оценки
            ...
            if state.current_subject_index < len(state.lessons):
                ...
                await update.message.reply_text(
                    f"📝 Какая оценка по {prep_subject}? (если оценки не было, напиши 'нет')"
                )
            else:
                ...
                valid_grades = [g for g in state.grades.values() if g is not None]
                ...
                if valid_grades:
                    ...
                    await update.message.reply_text(
                        "Сегодня не было оценок.\nВ следующий раз постарайся получить хорошие оценки!"
                    )
            ...
            if digits:
                ...
                await update.message.reply_text("Введи цифру от 1 до 10 или напиши 'нет'.")
...
async def handle_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # ========== ПОДТВЕРЖДЕНИЕ СПИСКА ==========
        ...
        if data == "confirm_yes":
            ...
            first_subject = state.lessons[state.current_subject_index]
