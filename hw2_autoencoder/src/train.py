...
# Добавляем пути для импорта наших модулей (пути к корневой папке проекта)
import sys

sys.path.append(os.path.dirname(os.path.dirname(__file__)))
# Корневая папка проекта (hw2_autoencoder/)
...
def create_results_folder():
    ...
    results_dir = os.path.join(BASE_DIR, 'results', f'run_{timestamp}')
...
def check_dimensions(encoder, decoder, autoencoder):
    ...
    print(f"После энкодера: {encoded.shape} → 8×8×32 (сжатое представление)")
...
def train_autoencoder(epochs=40, batch_size=128):
    """
